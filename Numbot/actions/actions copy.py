...
class GetProjectAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        suboption = [f"{index}. {project_name}" for index, project_name in enumerate(project_names, start=1)]
...
class GetAccessAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        access = [acobj['access_theme_name'] for obj in objkey for acobj in json_data[obj]]
        ...
        print(final_access)
        if access:
            dispatcher.utter_message(text="Select Access:")
            for index, level in enumerate(access, start=1):
                dispatcher.utter_message(text=f"{index}. {level}")

            suboption = [f"{index}. {level}" for index, level in enumerate(access, start=1)]
        ...
        return [SlotSet('submenu', "main"), SlotSet('suboption', None)]
...
class GetLayersAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if cat_names:
            ...
            suboption = [f"{index}. {level}" for index, level in enumerate(cat_names, start=1)]
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if 'data' in json_data and 'layers' in json_data['data']:
            ...
            if selected_layer:
                ...
                if attributes:
                    col_aliases = [f"{index}. {attr['col_alias']}" for index, attr in enumerate(attributes, start=1)]
                    dispatcher.utter_message('\n'.join(col_aliases))
                    slot_values = [f"{index}. {col_alias}" for index, col_alias in enumerate(col_aliases, start=1)]

                    # return [SlotSet("selected_attr", slot_values), SlotSet('suboption', None)]
                    return [SlotSet('submenu', "report_version")]

                else:
                    dispatcher.utter_message(f"No attributes found for the selected layer.")
            else:
                dispatcher.utter_message(f"No layer found for the selected option.")
        else:
